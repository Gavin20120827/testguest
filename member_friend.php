...
//         _location("很遗憾,没有任何数据被删除","member_message.php");
//     }
// }
...
_page('select tg_id from tg_friend where tg_touser="'.$_COOKIE['username'].'" or tg_fromuser="'.$_COOKIE['username'].'"',20);
// _page("select tg_id from tg_message where tg_touser='{$_COOKIE['username']}'",5);

$_result= _query("SELECT
                        tg_state,tg_id,tg_fromuser,tg_touser,tg_content,tg_date
                 FROM
                        tg_friend
...
<div id = "member">
    ...
    <div id = "member_message">
 	...
 	<?php 
	    ...
	    ?>
	    <tr><td><?php echo $_html['friend']?></td><td title="<?php echo $_html['content']?>"><?php echo _title($_html['content']) ?></td>
	    <td><?php echo $_html['date']?></td><td><?php echo $_html['state_html']?></td><td><input name="ids[]" value="<?php echo $_html['id']?>" type="checkbox" /></td></tr>
	<?php
         }
	?>
